...
def test_invalid_non_alphanumeric_key():
    assert parse_var_definition("BAD-VAR") == (None, None)

def test_invalid_chars_in_param():
    assert parse_var_definition("VAR(p@ram)") == (None, None)

def test_invalid_extra_text_after_parentheses():
    assert parse_var_definition("FUNC(a, b)) more") == (None, None)

def test_invalid_incomplete_param_block():
    assert parse_var_definition("FUNC(a, b") == (None, None)

def test_invalid_nested_parentheses():
    assert parse_var_definition("FUNC(a, b(1))") == (None, None)

def test_invalid_empty_param_list():
    assert parse_var_definition("FUNC()") == (None, None)
